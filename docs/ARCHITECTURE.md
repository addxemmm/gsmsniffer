...
Frequency selection is server-validated: each new observation carries originating job_id/band, the bounded catalog deduplicates by source scan and channel, and new captures reference scan_job_id. Legacy job history remains readable, but unlinked legacy observations cannot authorize a new capture.
